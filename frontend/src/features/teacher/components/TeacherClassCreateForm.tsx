...
import { AlertCircle, BookText, CalendarRange, Info, LoaderCircle, School, Shapes } from "lucide-react";
...
import { teacherYearLevelOptions } from "@/features/teacher/utils/teacher-class";
...
export function TeacherClassCreateForm({
  onSubmit,
  submitting,
}: {
  ...
  const form = useForm<TeacherClassCreateValues>({
    ...
    defaultValues: teacherClassCreateDefaultValues,
    mode: "onChange",
  });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const mapped = mapTeacherClassCreateSubmissionError(error);
      if (mapped.field) {
  ...
  return (
    <>
      ...
      >
        <div className="p-5 sm:p-6">
          <div className="mx-auto flex w-full max-w-5xl items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Maklumat ini digunakan semasa pendaftaran murid.</p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-border">
          <div className="mx-5 mt-5 rounded-xl border border-secondary/20 bg-secondary/5 p-4 sm:mx-6 sm:mt-6">
            <div className="mx-auto flex w-full max-w-5xl items-start gap-3">
              ...
              <div className="min-w-0">
                <h3 className="text-sm font-semibold text-foreground">Kelas akan digunakan untuk pendaftaran murid</h3>
                <p className="mt-1 text-sm leading-6 text-muted-foreground">
                  Rekod kelas ini akan tersedia semasa guru memilih kelas asal murid dalam proses pendaftaran.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6 p-5 sm:p-6">
            <div className="mx-auto w-full max-w-5xl space-y-6">
              <div className="w-full space-y-2">
                <Label htmlFor="teacher-class-year-level" className="text-sm font-semibold text-foreground">
                  Tahun <RequiredMark />
                </Label>
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <>
                      <Select value={field.value} onValueChange={field.onChange} disabled={submitting}>
                        <SelectTrigger
                          id="teacher-class-year-level"
                          aria-invalid={Boolean(fieldState.error)}
                          aria-describedby={fieldState.error ? "teacher-class-year-level-error" : undefined}
                          className={cn(
                      ...
                      <FieldError id="teacher-class-year-level-error" message={fieldState.error?.message} />
                    </>
                  )}
                />
              </div>
              ...
              <div className="w-full space-y-2">
                <Label htmlFor="teacher-class-name" className="text-sm font-semibold text-foreground">
                ...
                <div className="relative">
                  <Input
                    ...
                    aria-describedby={form.formState.errors.className ? "teacher-class-name-error" : "teacher-class-name-helper"}
                    className={cn(
                ...
                <FieldError id="teacher-class-name-error" message={form.formState.errors.className?.message} />
                <p id="teacher-class-name-helper" className="text-sm leading-6 text-muted-foreground">
                  Gunakan nama kelas seperti A, Bestari, Cemerlang atau Nilam.
                </p>
              </div>

              <div className="w-full space-y-2">
                <Label htmlFor="teacher-class-academic-year" className="text-sm font-semibold text-foreground">
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <>
                      <Select value={field.value} onValueChange={field.onChange} disabled={submitting}>
                        <SelectTrigger
                          id="teacher-class-academic-year"
                          aria-invalid={Boolean(fieldState.error)}
                          aria-describedby={fieldState.error ? "teacher-class-academic-year-error" : undefined}
                          className={cn(
                            "h-12 w-full rounded-xl bg-background/60 text-base focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20",
                            fieldState.error && "border-destructive focus-visible:border-destructive focus-visible:ring-destructive/20",
                          )}
                      ...
                      <FieldError id="teacher-class-academic-year-error" message={fieldState.error?.message} />
                    </>
                  )}
                />
              </div>
        ...
        <div className="w-full border-t border-border bg-muted/30 p-5 sm:p-6">
          <div className="mx-auto flex w-full max-w-5xl flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            >
              {submitting ? <LoaderCircle className="size-4 animate-spin" aria-hidden="true" /> : <BookText className="size-4" aria-hidden="true" />}
              {submitting ? "Mencipta..." : "Tambah Kelas"}
            </Button>
          </div>
        </div>
      </form>
      ...
      <AlertDialog open={discardOpen} onOpenChange={setDiscardOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Batalkan pendaftaran kelas?</AlertDialogTitle>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction asChild className="h-11 rounded-xl bg-secondary px-5 text-secondary-foreground hover:bg-secondary/90">
              <Link to="/guru/kelas">Buang Maklumat</Link>
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={(open) => !submitting && setConfirmOpen(open)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cipta kelas baharu?</AlertDialogTitle>
            <AlertDialogDescription>
              Kelas ini akan digunakan semasa pendaftaran murid untuk sekolah anda.
            </AlertDialogDescription>
          </AlertDialogHeader>

          <dl className="space-y-3 rounded-xl border border-border bg-muted/30 p-4">
            <div className="space-y-1">
              <dt className="text-sm font-semibold text-foreground">Tahun</dt>
              <dd className="text-sm text-muted-foreground">{pendingPayload ? `Tahun ${pendingPayload.yearLevel}` : "-"}</dd>
            </div>
            <div className="space-y-1">
              <dt className="text-sm font-semibold text-foreground">Nama Kelas</dt>
              <dd className="text-sm text-muted-foreground">{pendingPayload?.className ?? "-"}</dd>
            </div>
            <div className="space-y-1">
              <dt className="text-sm font-semibold text-foreground">Sesi Akademik</dt>
              <dd className="text-sm text-muted-foreground">{pendingPayload?.academicYear ?? "-"}</dd>
            </div>
          </dl>
      ...
      <span className="sr-only" role="status" aria-live="polite">{submitting ? "Kelas sedang ditambah." : ""}</span>
    </>
  );
}
