...
export function ReadingAudioControls({ media, showPlayAudio, showReplay, showPause, onAudioStart }: Props) {
  ...
  return <div className="space-y-3 rounded-xl border bg-card p-4"><audio ref={ref} controls className="w-full" preload="metadata" aria-label={media.altText ?? media.label ?? "Audio bacaan"}><source src={media.url} type={media.mimeType ?? undefined} />Pelayar anda tidak menyokong audio.</audio><div className="flex flex-wrap gap-2">{showPlayAudio ? <Button type="button" variant="outline" className="h-11" onClick={() => { void ref.current?.play() }}><Play /> Main audio</Button> : null}{showPause ? <Button type="button" variant="outline" className="h-11" disabled={!playing} onClick={() => ref.current?.pause()}><Pause /> Play audio</Button> : null}{showReplay ? <Button type="button" variant="outline" className="h-11" onClick={() => { if (!ref.current) return; ref.current.currentTime = 0; void ref.current.play() }}><RotateCcw /> Ulang audio</Button> : null}</div></div>
}
