...
const arrangeSyllablesMediaUrlPaths = [["arrangeSyllables", "media", "image", "url"], ["arrangeSyllables", "media", "audio", "url"]] as const;
...
function safeJson(value: unknown, code: string, maxBytes: number, allowedUrlPaths: readonly (readonly string[])[] = []): Prisma.InputJsonValue { if (value === null || jsonByteSize(value) > maxBytes) throw error(code, 400, "Konfigurasi aktiviti terlalu besar."); try { assertSafeMetadata(value, { allowedUrlPaths }); } catch { throw error(code, 400, "Konfigurasi aktiviti tidak selamat."); } return value as Prisma.InputJsonValue; }
...
function itemConfigurationAllowedUrlPaths(rendererKey: string): readonly (readonly string[])[] { return rendererKey === "arrange-syllables" ? arrangeSyllablesMediaUrlPaths : []; }
export function validateDigitalActivityItemConfigurationForPersistence(rendererKey: string, configuration: unknown): Prisma.InputJsonValue | Prisma.JsonNullValueInput { return configuration === null ? Prisma.JsonNull : safeJson(configuration, "DIGITAL_ACTIVITY_ITEM_INVALID", 16 * 1024, itemConfigurationAllowedUrlPaths(rendererKey)); }
...
export function distributeItemMarks(totalMarks: number, itemCount: number): number[] {
...
async function syncActivityItemMarks(
  tx: Prisma.TransactionClient,
  activityId: string,
  scoringMode: ActivityScoringMode,
  totalMarks: number | null,
): Promise<void> {
  const items = await tx.digitalActivityItem.findMany({
    where: { digitalActivityId: activityId },
  ...
  if (scoringMode === ActivityScoringMode.NONE || !totalMarks || totalMarks < 1) {
    await Promise.all(items.map((item) => tx.digitalActivityItem.update({
      where: { id: item.id },
      data: { marks: null },
    })));
    return;
  ...
  const distributedMarks = distributeItemMarks(totalMarks, items.length);
  await Promise.all(items.map((item, index) => tx.digitalActivityItem.update({
    where: { id: item.id },
    data: { marks: distributedMarks[index] ?? 1 },
  })));
}
...
export async function updateDigitalActivity(activityId: string, input: UpdateDigitalActivityBody, context: DigitalActivityAuditContext) { manage(context); const existing = await get(activityId); draft(existing); if (context.actor.role === UserRole.ADMIN && existing.createdByUserId !== context.actor.userId) throw denied(); const template = existing.activityTemplate; const configuration = input.configuration ?? existing.configuration; validateInstance(template.configurationSchema, configuration); const hasSettingsPayload = input.scoringMode !== undefined || input.reviewMode !== undefined || input.totalMarks !== undefined || input.masteryThreshold !== undefined || input.attemptsAllowed !== undefined || input.timeLimitSeconds !== undefined || input.shuffleItems !== undefined || input.showImmediateFeedback !== undefined || input.allowRetry !== undefined || input.rewardConfiguration !== undefined || input.presentationSettings !== undefined; const nextScoringMode = input.scoringMode ?? existing.scoringMode; const nextTotalMarks = input.totalMarks !== undefined ? input.totalMarks : existing.totalMarks; const updated = await prisma.$transaction(async (tx) => { const next = await tx.digitalActivity.update({ where: { id: activityId }, data: { ...(input.title !== undefined ? { title: input.title } : {}), ...(input.description !== undefined ? { description: input.description } : {}), ...(input.instructions !== undefined ? { instructions: input.instructions } : {}), ...(input.learningOutcome !== undefined ? { learningOutcome: input.learningOutcome } : {}), ...(input.difficulty !== undefined ? { difficulty: input.difficulty } : {}), ...(input.scoringMode !== undefined ? { scoringMode: input.scoringMode } : {}), ...(input.reviewMode !== undefined ? { reviewMode: input.reviewMode } : {}), ...(input.totalMarks !== undefined ? { totalMarks: input.totalMarks } : {}), ...(input.masteryThreshold !== undefined ? { masteryThreshold: input.masteryThreshold } : {}), ...(input.estimatedMinutes !== undefined ? { estimatedMinutes: input.estimatedMinutes } : {}), ...(input.attemptsAllowed !== undefined ? { attemptsAllowed: input.attemptsAllowed } : {}), ...(input.timeLimitSeconds !== undefined ? { timeLimitSeconds: input.timeLimitSeconds } : {}), ...(input.shuffleItems !== undefined ? { shuffleItems: input.shuffleItems } : {}), ...(input.showImmediateFeedback !== undefined ? { showImmediateFeedback: input.showImmediateFeedback } : {}), ...(input.allowRetry !== undefined ? { allowRetry: input.allowRetry } : {}), ...(input.configuration !== undefined ? { configuration: safeJson(input.configuration, "DIGITAL_ACTIVITY_CONFIGURATION_INVALID", 64 * 1024) } : {}), ...(input.rewardConfiguration !== undefined ? { rewardConfiguration: validateReward(input.rewardConfiguration) } : {}), ...(input.presentationSettings !== undefined ? { presentationSettings: validatePresentation(input.presentationSettings) } : {}), ...(hasSettingsPayload && !existing.settingsCompletedAt ? { settingsCompletedAt: new Date() } : {}), updatedByUserId: context.actor.userId }, include }); if (hasSettingsPayload) await syncActivityItemMarks(tx, activityId, nextScoringMode, nextTotalMarks); return tx.digitalActivity.findUniqueOrThrow({ where: { id: activityId }, include }); }); await recordAuditEvent(audit(context, "DIGITAL_ACTIVITY_UPDATED", activityId, safeAudit(existing), safeAudit(updated))); return dto(updated); }
export async function deleteDigitalActivity(activityId: string, context: DigitalActivityAuditContext): Promise<void> { manage(context); const existing = await get(activityId); draft(existing); if (context.actor.role === UserRole.ADMIN && existing.createdByUserId !== context.actor.userId) throw denied(); const assignmentCount = await prisma.assignment.count({ where: { digitalActivityId: activityId } }); if (assignmentCount > 0) throw error("DIGITAL_ACTIVITY_DELETE_BLOCKED", 409, "Aktiviti ini tidak boleh dipadam kerana sedang digunakan."); await prisma.digitalActivity.delete({ where: { id: activityId } }); await recordAuditEvent(audit(context, "DIGITAL_ACTIVITY_DELETED", activityId, safeAudit(existing), null)); }
...
async function validateItem(record: ActivityRecord, input: AddDigitalActivityItemBody): Promise<ValidatedActivityItemContract> {
  ...
  if (input.configuration !== undefined && input.configuration !== null) validateDigitalActivityItemConfigurationForPersistence(record.activityTemplate.rendererKey, input.configuration);
...
export async function addDigitalActivityItem(activityId: string, input: AddDigitalActivityItemBody, context: DigitalActivityAuditContext) {
  ...
  try {
    const item = await prisma.$transaction(async (tx) => {
      const created = await tx.digitalActivityItem.create({
        data: {
          ...
          ...(input.configuration !== undefined ? { configuration: validateDigitalActivityItemConfigurationForPersistence(record.activityTemplate.rendererKey, input.configuration) } : {}),
        },
      });
      await syncActivityItemMarks(tx, activityId, record.scoringMode, record.totalMarks);
      return tx.digitalActivityItem.findUniqueOrThrow({ where: { id: created.id } });
    });
...
type DigitalActivityPublishReadiness = {
  ready: boolean;
  issues: string[];
  checks: {
    information: boolean;
...
async function collectWorkflowIssues(record: ActivityRecord, publication: boolean): Promise<string[]> {
  const issues: string[] = [];
  ...
  if (
    publication
    && record.status === DigitalActivityStatus.IN_REVIEW
    && !record.reviewHistory.some((entry) => entry.toStatus === DigitalActivityStatus.IN_REVIEW)
  ) {
    issues.push("REVIEW_HISTORY_REQUIRED");
  }
  return [...new Set(issues)];
}

function buildReadinessChecks(issues: readonly string[]): DigitalActivityPublishReadiness["checks"] {
  const hasIssue = (codes: readonly string[]) => codes.some((code) => issues.includes(code));
  return {
    information: !hasIssue(["CONTENT_REQUIRED", "CONFIGURATION_INVALID"]),
    curriculum: !hasIssue(["CURRICULUM_NOT_PUBLISHED", "CURRICULUM_LINK_REQUIRED", "PRIMARY_LINK_REQUIRED"]),
    content: !hasIssue([
      "ITEM_REQUIRED",
      "ITEM_ORDER_INVALID",
      "ITEM_INVALID",
      "FILL_BLANK_CONTRACT_INVALID",
      "ARRANGE_LETTERS_CONTRACT_INVALID",
      "ARRANGE_SYLLABLES_CONTRACT_INVALID",
      "WORD_BUILDER_CONTRACT_INVALID",
      "TRACING_CONTRACT_INVALID",
      "COPY_WRITING_CONTRACT_INVALID",
      "READING_CONTRACT_INVALID",
      "FREE_HANDWRITING_CONTRACT_INVALID",
      "READING_COMPREHENSION_CONTRACT_INVALID",
      "VOICE_RECORDING_CONTRACT_INVALID",
      "MEDIA_INVALID",
    ]),
    settings: !hasIssue(["SCORING_INVALID", "CONFIGURATION_INVALID"]),
    preview: !hasIssue([
      "CONTENT_REQUIRED",
      ...
      "SCORING_INVALID",
      "MEDIA_INVALID",
      "REVIEW_HISTORY_REQUIRED",
    ]),
  };
}

async function reviewValidation(record: ActivityRecord, publication: boolean): Promise<void> {
  const issues = await collectWorkflowIssues(record, publication);
  if (issues.length) throw error(publication ? "DIGITAL_ACTIVITY_PUBLICATION_INVALID" : "DIGITAL_ACTIVITY_REVIEW_INVALID", 400, "Aktiviti belum memenuhi syarat workflow.", { issues });
}
export async function getDigitalActivityPublishReadiness(activityId: string, context: DigitalActivityAuditContext): Promise<DigitalActivityPublishReadiness> {
  const record = await get(activityId);
  visible(record, context);
  const issues = await collectWorkflowIssues(record, true);
  return {
    ready: issues.length === 0,
    issues,
    checks: buildReadinessChecks(issues),
  };
}
...
export async function publishDigitalActivity(activityId: string, context: DigitalActivityAuditContext) { superAdmin(context); const record = await get(activityId); if (record.status !== DigitalActivityStatus.IN_REVIEW && record.status !== DigitalActivityStatus.DRAFT && record.status !== DigitalActivityStatus.ARCHIVED) throw transition(); await reviewValidation(record, true); const updated = await prisma.$transaction(async (tx) => { const next = await tx.digitalActivity.update({ where: { id: activityId }, data: { status: DigitalActivityStatus.PUBLISHED, publishedAt: new Date(), archivedAt: null, updatedByUserId: context.actor.userId }, include }); await tx.digitalActivityReviewHistory.create({ data: { digitalActivityId: activityId, actorUserId: context.actor.userId, fromStatus: record.status, toStatus: DigitalActivityStatus.PUBLISHED } }); await recordAuditEvent(audit(context, "DIGITAL_ACTIVITY_PUBLISHED", activityId, safeAudit(record), safeAudit(next)), { transactionClient: tx, strict: true }); return next; }); return dto(updated); }
