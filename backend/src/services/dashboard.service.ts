...
export async function getSuperAdminDashboard(context: DashboardContext, query: DashboardQuery) {
  ...
  const [totalSchools, activeSchools, suspendedSchools, archivedSchools, totalAdmins, activeAdmins, pendingAdmins, totalTeachers, totalStudents, totalParents, totalClasses, schools, admins, teachers] = await Promise.all([
    prisma.school.count(),
  ...
  return { summary: { totalSchools, activeSchools, suspendedSchools, archivedSchools, totalAdmins, activeAdmins, pendingAdmins, totalTeachers, totalStudents, totalParents, totalClasses }, recent: { schools: schools.map(recentSchool), admins: admins.map(recentAdmin), teachers: teachers.map(recentTeacher) } };
}
...
export async function getAdminDashboard(context: DashboardContext, query: DashboardQuery) {
  ...
  const [totalSchools, activeSchools, totalTeachers, activeTeachers, pendingTeachers, totalStudents, activeStudents, totalParents, totalClasses, schools, teachers, students, classCapacity] = await Promise.all([
  ...
  return { summary: { totalSchools, activeSchools, totalTeachers, activeTeachers, pendingTeachers, totalStudents, activeStudents, totalParents, totalClasses }, recent: { schools: schools.map(recentSchool), teachers: teachers.map(recentTeacher), students: students.map(recentStudent) }, classesNearingCapacity: nearCapacity };
}
...
export async function getTeacherDashboard(context: DashboardContext, query: DashboardQuery) {
  ...
  const [classes, activeStudents, suspendedStudents, students] = await Promise.all([
...
export async function getParentDashboard(context: DashboardContext) {
  ...
  return { parent: { id: parent.id, fullName: parent.fullName, phone: parent.phone, occupation: parent.occupation, avatar: parent.avatar, accountStatus: parent.user.accountStatus }, linkedChildrenCount: parent.students.length, children: parent.students.map((link) => ({ id: link.student.id, studentId: link.student.studentId, fullName: link.student.fullName, gender: link.student.gender, avatar: link.student.avatar, accountStatus: link.student.user.accountStatus, relationship: link.relationship, school: schoolSummary(link.student.school), class: link.student.class })) };
}
